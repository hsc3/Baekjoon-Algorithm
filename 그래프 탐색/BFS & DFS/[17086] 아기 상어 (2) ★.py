...
for i in range(N):
    ...
    for j in range(len(row)):
        # 상어의 위치인 경우, 상어의 위치를 큐에 넣고, 1 -> 0 변경 (이동 거리인 0으로)
        if row[j] == 1:
            queue.append([i, j])
            row[j] = 0
        # 상어의 위치가 아니면, 무한히 큰 값 저장
        else:
            row[j] = float("inf")
    arr.append(row)
...
while queue:
    ...
    answer = max(answer, arr[x][y]) # 최대 안전 거리 저장

    for i in range(8):
        xx = x + dx[i]
        yy = y + dy[i]
        # 최단 거리로 해당 지점을 방문할 수 있는 경우에 갱신
        if 0 <= xx < N and 0 <= yy < M and arr[xx][yy] > arr[x][y] + 1:
            arr[xx][yy] = arr[x][y] + 1
            queue.append([xx, yy])
